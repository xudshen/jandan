...
/**
 * from (https://github.com/toddway/MaterialTransitions)
...
public class TransitionHelper {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            ...
            activity.getWindow().getSharedElementEnterTransition().addListener(new Transition.TransitionListener() {
                ...
                public void onTransitionStart(Transition transition) {
                    if (isAfterEnter())
                        for (Listener listener : listeners) listener.onBeforeReturn();
                }
    ...
    public void onViewCreated() {
        ...
        if (!isAfterEnter()) {
            for (Listener listener : listeners) listener.onBeforeEnter(contentView);
        }
